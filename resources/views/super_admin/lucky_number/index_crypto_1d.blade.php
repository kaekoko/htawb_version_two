...
@section('title', 'CRYPTO 1D Lucky Number')
        ...
        <div class="p-2">
            <div class="form-group">
                <div class="input-group">
                    ...
                    <input type="text" name="bet_date" class="form-control form-control-sm date"
                        value="{{ request()->date ?? date('Y-m-d') }}">
                </div>
            </div>
        </div>
        <div class="p-2">
            <a href="{{ url('/super_admin/lucky_number_c1d/create') }}" class="btn btn-dark btn-sm" title="Add Lucky Number">
                            ...
                            <tbody>
                                @foreach ($lucky_numbers as $item)
                                    <tr>
                                        ...
                                        <td>
                                            @if ($item->approve == 0)
                                               <form action="{{url('super_admin/approveC1d')}}" method="POST">
                                                @csrf
...
@section('scripts')
    <script>
        ...
        $('.reload').click(function() {
            window.location = '/super_admin/lucky_number_c1d';
        });
    </script>
@endsection
