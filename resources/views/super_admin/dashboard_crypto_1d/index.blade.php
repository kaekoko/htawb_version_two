...
@section('title', 'Super Admin Crypto 1D Dashboard')

@section('dashboard-crypto-1d-active', 'active')
        ...
        <div class="col-12">
            <div class="tab-content" id="accountContent">
                ...
                <div class="row justify-content-center mt-3 clerance_div">
                    <a href="{{url('/super_admin/dashboard_c1d')}}">
                ...
                <div class="row justify-content-center mt-3 all_bet">
                    ...
                    <div class="p-2">
                        ...
                        @endphp
                        <button type="button" class="btn btn-block btn-lg custom_dashboard{{ $isBlocked ? ' bg-danger active disabled' : '' }} {{ $isHot && $betAmount >= $hot_amount ? 'bg-danger' : '' }} {{ $betAmount < $overAll->over_all_amount_crypto_1d ? '' : 'bg-danger active' }} {{ $betAmount == 0 ? 'bg-secondary' : '' }}" data-toggle="modal" data-target="{{ $isBlocked ? '' : '#example' . $t->bet_number }}">
                        ...
                        <table class="table table-striped table-sm">
                            ...
                            <tbody id="bet_slip">
                                ...
                                <tr>
                                    ...
                                    <td>{{$b->total_bet}} </td>
                                    <td>{{$b->date}}</td>
                                    <td><a href="{{url('super_admin/bet_slip_c1d',$b->id)}}" title="View"><button class="btn btn-info btn-sm"><i class="fa fa-eye"></i></button></a></td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
...
<div class="modal fade modal-password" id="clearance" tabindex="-1" role="dialog" aria-labelledby="clearance" aria-hidden="true">
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="POST" enctype="multipart/form-data" action="{{ url('super_admin/clearance_c1d_money') }}">
                    @csrf
                    <div class="form-group">
...
<div class="modal fade modal-refund" id="refund" tabindex="-1" role="dialog" aria-labelledby="clearance" aria-hidden="true">
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="POST" enctype="multipart/form-data" action="{{ url('super_admin/refund_money_c1d') }}">
                    @csrf
                    <div class="form-group">
                        <input type="password" name="password" class="form-control" required>
                        <input type="hidden" name="section" value="{{$section->time_section}}" class="pass_section">
...
<script>
    ...
    $('.reload').click(function() {
        ...
        Swal.fire({
            ...
            html: "Waiting to the Crypto 1D Dashboard",
            timer: 20000,
