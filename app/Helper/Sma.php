...
class Sma{
        ...
        $result = [];
            if($req_who === "senior"){
                if($column === 'master_agent_id'){
                    ...
                    $data = Sma::findAgent($date, $sma_id, 'senior_agent_id');
                    $result = $data;
                } else {
                    $sma_users = User::where('senior_agent_id', $sma_id)->get();
                    foreach($sma_users as $sma_user){
                        $bettings = UserBet::where('user_id', $sma_user->id)->where('date', $date)->get();
                        if($bettings->count() > 0){
                ...
                if($column !== 'user'){
                    $data = Sma::findAgent($date, $sma_id, 'master_agent_id');
                    $result = $data;
                } else {
    ...
    public static function sma_record_section($time,$date, $column, $sma_id, $req_who, $id){
        ...
        if($req_who === "senior"){
            if($column === 'master_agent_id'){
                ...
                $data = Sma::findAgentSection($time,$date, $id);
                array_push($result, $data);
            } else {
                $sma_users = User::where('senior_agent_id', $sma_id)->get();
                foreach($sma_users as $sma_user){
                    $bettings = UserBet::where('user_id', $sma_user->id)->where('date', $date)->where('section', $time)->get();
                    if($bettings->count() > 0){
                        $reward = $bettings->sum('reward_amount');
                        $bet_amount = $bettings->sum('total_amount');
                        $res = [
    ...
    public static function sma_grant_numbers($time, $date, $id, $column){
        ...
        if($column == 'senior_agent_id'){
            ...
            $userbets = UserBet::whereIn('user_id', $last_users)->where('date', $date)->where('section', $time)->with('bettings')->get();
            if($userbets->count() > 0){
                ...
                foreach($numbers as $num){
                    $betting_amount = Betting::whereIn('id', $betslip)->get();
                    ...
                    if($num->hot_amout_limit > 0 ){
                        ...
                        if($overall->over_all_amount <= $total_amount){
                            $message = 1;
                        } else {
                            $message = 0;
                        }
    ...
    public static function sma_grant_numbers_daily($date, $id, $column){
        ...
        if($column == 'senior_agent_id'){
            ...
            $userbets = UserBet::whereIn('user_id', $last_users)->where('date', $date)->with('bettings')->get();
            if($userbets->count() > 0){
                foreach($userbets as $bet){
                    foreach($bet->bettings as $b){
                        array_push($betslip, $b->id);
    ...
    public static function sma_single_detail($time, $date, $number,$column, $id){
        ...
        if($column == 'senior_agent_id'){
            ...
            $userbets = UserBet::whereIn('user_id', $last_users)->where('section', $time)->where('date', $date)->with('bettings')->get();
            foreach($userbets as $bet){
                foreach($bet->bettings as $b){
                    array_push($betslip, $b->id);
                }
            ...
            $bettings = Betting::whereIn('id', $betslip)->where('bet_number', $number)->with('user_bets')->get();
        } else if($column == 'master_agent_id'){
            $arr = [
                [
                "id" => 'master_agent_id',
    ...
    public static function sma_section_statics($time,$date,$column, $id){
        ...
        if($column == 'senior_agent_id'){
            ...
            $userbets = UserBet::whereIn('user_id', $last_users)->where('date', $date_format)->where('section', $time)->with('bettings')->get();
            foreach($userbets as $bet){
                foreach($bet->bettings as $b){
                    array_push($betslip, $b->id);
                }
            }
            $bet_counts = Betting::where('section', $time)->where('date', $date_format)->whereIn('id', $betslip)->get();
            ...
            $userbets = UserBet::whereIn('user_id', $last_users)->where('section', $time)->where('date', $date_format)->with('bettings')->get();
            foreach($userbets as $bet){
                foreach($bet->bettings as $b){
                    array_push($betslip, $b->id);
                }
    ...
    public static function sma_daily_statics($date,$column, $id){
        ...
        if($column == 'senior_agent_id'){
            ...
            $userbets = UserBet::whereIn('user_id', $last_users)->where('date', $date_format)->with('bettings')->get();
            foreach($userbets as $bet){
                foreach($bet->bettings as $b){
                    array_push($betslip, $b->id);
                }
            }
            $bet_counts = Betting::where('date', $date_format)->whereIn('id', $betslip)->get();
            $bet_amounts = $userbets->sum('total_amount');
            $reward_amounts = $userbets->sum('reward_amount');
