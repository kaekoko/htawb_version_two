...
class Dashboard3dController extends Controller
{
    ...
    private function getBlockOrHotAmount($type)
    {
        $data = NumberSetting3d::where('type', $type)
            ->latest()
            ->first();
    ...
    public function section(Request $request, $section)
    {
        ...
        $hot_number =  $this->getBlockOrHotData('hot', $date);
        $hot_amount = $this->getBlockOrHotAmount('hot', $date);
        ...
        return view('super_admin.dashboard.index_3d', compact('t_month', 'approve', 'read', 'numberLucky', 'total_amount', 'betting', 'cur_month', 'cur_year', 'months', 'years', 'two_lottery_off', 'two_ds', 'section', 'block_number', 'hot_number', 'hot_amount', 'betting_total', 'overAll'));
    }
