...
class BetSlipController extends Controller
{
    ...
    {
        $bet_slips = UserBet::with('bettings');
        ...
        return view('super_admin.bet_slip.bet_slip',compact('bet_slips','sections'));
    }

    public function bet_slip_detail($id)
    {
        $bet_slip_detail = UserBet::findOrFail($id);
        $bet_slip_detail->read = 1;
        $bet_slip_detail->update();

        return view('super_admin.bet_slip.bet_slip_detail',compact('bet_slip_detail'));
    }

    public function pay_reward($id,$amount)
    {
        $pay_reward = UserBet::findOrFail($id);
        $pay_reward->reward = 1;
        $pay_reward->update();
        ...
        return redirect("super_admin/bet_slip/$id")->with('flash_message','Pay Reward Successfully');
    }
    ...
    public function all_bets(Request $request){
        ...
        if(HolidayHelper::isTheDayHoliday($date, $lot_off) === 1){
            ...
            return helper::time_filter($time, $date);
        }
    }
    ...
    public function single_bet_detail(Request $request){
        ...
        $curdate = gmdate('Y-m-d', time()+$diffWithGMT);
        if($date !==  null){
            return helper::single_detail($time, $date, $number);
        }
        return helper::single_detail($time, $curdate, $number);
    }
    ...
    public function daily_bet_detail(Request $request){
        ...
        $curdate = gmdate('Y-m-d', time()+$diffWithGMT);
        if($date !==  null){
            return helper::daily_detail($date, $number);
        }
        return helper::daily_detail($curdate, $number);
    }
    ...
    public function daily_all_bets(Request $request){
        ...
        if(HolidayHelper::isTheDayHoliday($date, $lot_off) === 1){
            ...
            return helper::daily_filter($date);
        }
    }
