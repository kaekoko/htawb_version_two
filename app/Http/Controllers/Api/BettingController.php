...
class BettingController extends Controller
{
    public function betting(Request $request)
    {
        ...
        $twodoverallamount = $over_all_setting[0]["over_all_amount"];

        $get_latest_sec = Section::where("is_open", 1)
            ->where("time_section", $timesection)
            ->latest("id")
            ->first();

        $total_twod_amount = Betting::select("bet_number", DB::raw("SUM(amount) as amount"))
            ->where("section", $request->section)
            ->where("date", $sdate)
            ->groupBy("bet_number")
            ->get();

        $twodhots = NumberSetting::where('section', $request->section)->where('type', 'hot')->first();
        $twodhotamount = $twodhots->hot_amount;
        $hotsnumbers = explode(',', $twodhots->hot_number);
        ...
        $two_d_odd = Section::where("is_open", 1)
            ->where("time_section", $timesection)
            ->first();
                    ...
                    if ($check_amount <= $user->balance) {
                        if ($request->bet_obj) {
                            ...
                            foreach ($request->bet_obj as $value) {
                                ...
                                //overallsteeing
                                if (!empty($total_twod_amount)) {
                                    foreach ($total_twod_amount as $key => $oldvalue) {
                                        if (!empty($hotsnumbers)) {
                                            if (in_array($value["bet_number"], $hotsnumbers) && $value["bet_number"] == $oldvalue->bet_number) {
                                                $hotbetamount = $value["amount"] + $oldvalue->amount;
                                                if ($hotbetamount > $twodhotamount) {
                                                    return response()->json(
                                                        [
                                                            "message" => $value["bet_number"] . "(avaliable hot amount)" . $twodhotamount - $oldvalue->amount,
                                                        ],
                                                        400
                                                    );
                                                }
                                        ...
                                        //hotnumber
                                        if ($value["bet_number"] == $oldvalue->bet_number) {
                                            $ttbetamount = $value["amount"] + $oldvalue->amount;
                                            if ($ttbetamount > $twodoverallamount) {
                                                return response()->json(
                                                    [
                                                        "message" => $value["bet_number"] . "(avaliable amount)" . $twodoverallamount - $oldvalue->amount,
                                                    ],
                                                    400
                                                );
                                            }
                        ...
                        if ($user->agent_id) {
                            ...
                            if (!empty($agent)) {
                                invokeAll::agent_referral_history(
                                    $user->id,
                                    $agent->id,
                                    $total_amount,
                                    $request->section,
    ...
    public function betting_1d(Request $request)
    {
        ...
        $get_latest_sec = Section1d::where("is_open", 1)
            ->where("time_section", $timesection)
            ->latest("id")
            ->first();
        if (empty($get_latest_sec)) {
                    ...
                    if ($check_amount <= $user->balance) {
                        ...
                        if ($user_bet->save()) {
                            ...
                            foreach ($bettings as $key => $value) {
                                ...
                                $betting->amount = $value["amount"];
                                $betting->odd =
                                    $over_all_setting[0]->over_all_odd_1d;
    ...
    public function betting_c2d(Request $request)
    {
        ...
        $get_latest_sec = SectionCrypto2d::where("time_section", $timesection)
            ->latest("id")
            ->first();
        if (empty($get_latest_sec)) {
            return response()->json(
    ...
    public function betting_c1d(Request $request)
    {
        ...
        $timesection = date("H:i", strtotime($request->section));
        // $get_latest_sec = SectionCrypto2d::latest('id')->first();
        $get_latest_sec = Sectionc1d::where("is_open", 1)
            ->where("time_section", $timesection)
            ->latest("id")
            ->first();
        if (empty($get_latest_sec)) {
    ...
    public function betting_3d(Request $request)
    {
        ...
        if ($user->balance > 0) {
            ...
            if ($check_amount <= $user->balance) {
                ...
                if ($user->referral_id) {
                    $type = "3D";
                    invokeAll::user_referral_history_3d(
                        $user->id,
                        $user->referral_id,
                        $total_amount,
                        $date3d,
                        $over_all_setting[0]->referral_3d,
                        $type,
                        $bet_date
                    );
                }
